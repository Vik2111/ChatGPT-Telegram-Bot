...
def get_python_executable(command: str) -> str:
    ...
    Returns:
        str: Путь к исполняемому файлу Python.
    """
...
def excute_command(command):
    """
    Выполнить команду и вернуть результат (стандартный вывод печатается в консоль в реальном времени).

    Важные правила:
    - **Клонирование репозиториев**: При использовании `git clone` необходимо указывать новый подкаталог в качестве цели, чтобы избежать ошибок в непустом рабочем каталоге.
      - **Правильный пример**: `git clone https://github.com/user/repo_name.git repo_name`
      - **Неправильный пример**: `git clone https://github.com/user/repo_name.git .`
    - **Запрещено**: Запрещено использовать для просмотра PDF, запрещено использовать команду `pdftotext`.
    - **Проверка статуса подзадач**: Запрещено использовать `ls`, `cat` и другие команды файловой системы для циклической проверки статуса выполнения подзадач или их вывода. Вместо этого используйте инструменты типа `get_task_result`, `get_all_tasks_status` для получения статуса и результатов подзадач.

    Параметры:
        command: Команда для выполнения, например, клонирование репозитория, установка зависимостей, запуск кода и т. д.

    Возвращает:
        Финальный статус выполнения команды и собранную информацию о выводе/ошибках.
    """
    try:
        ...
        if process.returncode == 0:
            ...
            # Если это режим PTY, stderr уже включен в final_output_log
            if IS_UNIX:
                return f"<tool_error>Ошибка выполнения команды (код выхода {process.returncode}):\nВывод/Ошибка:\n{final_output_log}</tool_error>"
            else:
                return f"<tool_error>Ошибка выполнения команды (код выхода {process.returncode}):\nОшибка: {stderr_output}\nВывод: {final_output_log}</tool_error>"

    except FileNotFoundError:
        return f"<tool_error>Ошибка выполнения команды: команда или программа не найдена ({command})</tool_error>"
    except Exception as e:
        return f"<tool_error>Исключение при выполнении команды: {e}</tool_error>"
