...
def url_to_markdown(url):
    ...
    def get_body(url):
        try:
            ...
            if text == "":
                return "Извините, в данный момент веб-страница недоступна."
...
def jina_ai_Web_crawler(url: str, isSearch=False) -> str:
    ...
    try:
        ...
        if response.status_code == 404:
            ...
            return "Извините, страница не найдена, в данный момент она недоступна. @Trash@"
        ...
        if result == '' and not isSearch:
            result = "Извините, возможно, сработала защита от роботов, в данный момент страница недоступна. @Trash@"
...
def get_url_content(url: str) -> str:
    """
    Получить содержимое веб-страницы по URL в формате markdown.
    ...
    def score_content(content):
        ...
        # 2. 是否包含错误信息
        error_penalty = 1000 if "Извините" in content or "@Trash@" in content else 0
...
def getgooglesearchurl(result, numresults=3):
    ...
    try:
        ...
        if "rateLimitExceeded" in str(e):
            print("Лимит запросов Google API исчерпан на сегодня, пожалуйста, попробуйте завтра!")
...
async def get_url_text_list(keywords, search_url_num):
    ...
    print(f"Время поиска: {run_time} секунд")
